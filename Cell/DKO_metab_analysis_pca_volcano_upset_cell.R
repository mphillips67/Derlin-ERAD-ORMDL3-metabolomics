...
# cellular extract only
data <- data %>% select(1, contains("Cell", ignore.case = FALSE))

cell_cols <- names(data)[-1]
data[cell_cols] <- lapply(data[cell_cols], as.double)
...
print(p_pca)
ggsave("PCA_Cell.svg", plot = p_pca, device = "svg", width = 8, height = 6, dpi = 300)
...
# Upregulated
svg("UpSet_Upregulated_Cell.svg", width = 10, height = 6, pointsize = 12)
...
# Downregulated
svg("UpSet_Downregulated_Cell.svg", width = 10, height = 6, pointsize = 12)
...
print(p_volc)
ggsave("Volcano_AllContrasts_Cell.svg", plot = p_volc, device = "svg",
